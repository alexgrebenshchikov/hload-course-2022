...
func OpenSQLConnection() DbInteractor {
	...
	conn, _ := sql.Open("postgres", "postgres://postgres:postgres@localhost:5432/alex-greben?sslmode=disable")
...
func (db *DbInteractor) CreateClicksCountTableIfNotExists() {
	rows, err := db.Conn.Query("CREATE TABLE IF NOT EXISTS clicks_count (short_url text primary key, cnt int)")
	if err != nil {
		fmt.Println("Unable to create table")
	}
	defer rows.Close()
...
func (db *DbInteractor) UpdateCounter(short_url string, cnt int) {
	rows, err := db.Conn.Query(fmt.Sprintf("INSERT INTO clicks_count values ('%s', %d) ON CONFLICT(short_url) DO UPDATE SET cnt=clicks_count.cnt+%d", short_url, cnt, cnt))
	if err != nil {
